...
export function LiveLinkRequestContext({ onUse }: { onUse: (machineReference: string, text: string) => void }) {
  ...
  const [provider, setProvider] = useState("JCB LiveLink");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const response = await fetch(`/api/integrations/${source}/context?${params}`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal });
  ...
  return <section className="rounded-xl border border-slate-300 bg-slate-50 p-4 text-slate-900" aria-label="LiveLink parts request context">
    <h2 className="font-semibold">Parts request from {provider}</h2>
