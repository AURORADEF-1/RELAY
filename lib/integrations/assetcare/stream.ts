...
export async function streamRequest(key:string,id?:string,fetcher:typeof fetch=fetch):Promise<Batch|null>{
 ...
 try{
  ...
  if(!response.ok){await response.body?.cancel();const retry=response.headers.get('Retry-After'),seconds=retry&&/^\d+$/.test(retry)?Number(retry):retry?Math.ceil((Date.parse(retry)-Date.now())/1000):300;throw new StreamError(response.status,Math.min(86400,Math.max(300,Number.isFinite(seconds)?seconds:300)));}
...
export async function collectCycle(options:{key:string;ownerId:string;save:(hash:string,items:unknown[],assets:AssetCareSnapshot[])=>Promise<void>;acknowledged:()=>Promise<void>;poll?:typeof streamRequest;now?:()=>number;maxBatches?:number}){
 const poll=options.poll??streamRequest,now=options.now??Date.now,deadline=now()+90000;let batches=0,records=0;
 while(batches<(options.maxBatches??10)&&now()<deadline-55000){
  const batch=await poll(options.key);if(!batch||!batch.items.length)return {batches,records,drained:true};
  ...
  await poll(options.key,batch.id!);await options.acknowledged();batches++;records+=batch.items.length;
 }
 return {batches,records,drained:false};
}
