...
async function request(path:"unit"|"GetUnitExtendedInfo"|"report/unitActiveFaults", params:Record<string,string>={}) {
  ...
  try {
    const response=await fetch(url,{redirect:"error",cache:"no-store",signal:AbortSignal.timeout(20000),headers:{Accept:"application/json"}});
...
export async function fetchTrackunitFleet(){
  const parsed=z.object({list:z.array(unitSchema).max(10000)}).safeParse(await request("unit"));
...
export const getTrackunitFleet=unstable_cache(fetchTrackunitFleet,["trackunit-fleet-v1"],{revalidate:900});
export async function fetchTrackunitDetails(pin:string){
  ...
  const telemetry=values.status==='fulfilled'?z.object({result:z.array(telemetrySchema)}).safeParse(values.value):null;
  const faults=reports.status==='fulfilled'?z.object({list:z.array(faultSchema)}).safeParse(reports.value):null;
