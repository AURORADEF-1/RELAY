...
export type Preferences={brand:string;category:string;group:string;providers:string[];yard:boolean;labels:boolean;cluster:boolean;base:'map'|'satellite';freshness:'all'|'fresh'|'old'|'missing';status:'all'|'fault'|'movement'|'running'|'unknown';area:'all'|'yard'|'away'};
