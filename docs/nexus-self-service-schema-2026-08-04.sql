...
create policy "nexus self service admin read"
...
begin
  if actor_id is null or not exists (
    select 1 from public.profiles p where p.id = actor_id and p.role = 'admin'
  ) then
  ...
  select coalesce(nullif(btrim(p.full_name), ''), split_part(u.email, '@', 1), 'Administrator')
    into actor_name
  from auth.users u
  ...
  insert into public.tickets (
    ...
    request_summary,
    'PENDING',
    'Created through the admin-only NEXUS Stores Self-Service workflow.'
...
comment on table public.nexus_self_service_requests is 'Idempotent RELAY outbox for confirmed admin-only NEXUS stores issues.';
comment on function public.create_nexus_self_service_ticket(uuid, uuid, jsonb) is 'Admin-only atomic creation of a RELAY ticket and pending NEXUS part rows.';
comment on function public.finalize_nexus_self_service_ticket(uuid, uuid, jsonb) is 'Admin-only reconciliation of the idempotent NEXUS allocation result into RELAY.';
