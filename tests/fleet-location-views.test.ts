...
it('pins exact coordinates in map views and keeps Street View separate',()=>{
 ...
 for(const [index,layer] of [[2,'k']] as const){const url=new URL(links[index].href);expect(url.searchParams.get('q')).toBe('52.39159,0.95505');expect(url.searchParams.get('t')).toBe(layer);expect(url.searchParams.has('center')).toBe(false);}
 expect(links[3].href).toContain('/place/52.39159%2C0.95505/');expect(links[3].href).toContain('!3d52.39159!4d0.95505!5m1!1e4');
});
