...
import { authorizeTakeuchi as authorizeJcb } from "@/lib/integrations/takeuchi/server";
...
describe("Takeuchi access boundary", () => {
  beforeEach(() => { process.env.TAKEUCHI_ENABLED = "true"; });
  afterEach(() => { delete process.env.TAKEUCHI_ENABLED; vi.clearAllMocks(); });
  ...
  it("fails closed until explicitly enabled", async () => { client("admin", false); delete process.env.TAKEUCHI_ENABLED; await expect(authorizeJcb(request)).rejects.toMatchObject({ status: 503 }); });
