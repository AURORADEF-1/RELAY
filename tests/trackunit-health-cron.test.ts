...
vi.mock('@/lib/integrations/trackunit/client',()=>({getTrackunitFleet:m.fleet,getTrackunitDetails:m.faults}));
...
import {GET} from '@/app/api/cron/trackunit-health/route';
import {validCronAuthorization} from '@/lib/integrations/jcb/cron-auth';
const req=(secret='secret')=>new NextRequest('http://localhost/api/cron/trackunit-health',{headers:{authorization:`Bearer ${secret}`}});
beforeEach(()=>{vi.clearAllMocks();vi.stubEnv('CRON_SECRET','secret');vi.stubEnv('TRACKUNIT_HEALTH_ALERTS_ENABLED','true');vi.stubEnv('TRACKUNIT_ENABLED','true');vi.stubEnv('NEXT_PUBLIC_SUPABASE_URL','https://example.test');vi.stubEnv('JCB_HEALTH_DATABASE_KEY','server-test-key');const chain={select:()=>chain,order:()=>chain,limit:()=>chain,maybeSingle:async()=>({data:null,error:null})};m.client.mockReturnValue({rpc:m.rpc,from:()=>chain});m.rpc.mockResolvedValue({data:2,error:null});m.fleet.mockResolvedValue({machines:[{pin:'P1',equipmentId:'1',model:'535',position:null}]});m.faults.mockResolvedValue({faults:[],telemetry:[],faultError:false,telemetryError:false});});
...
it('does not scan or send until monitoring is explicitly enabled',async()=>{vi.stubEnv('TRACKUNIT_HEALTH_ALERTS_ENABLED','false');expect(await(await GET(req())).json()).toEqual({enabled:false});expect(m.client).not.toHaveBeenCalled();});
