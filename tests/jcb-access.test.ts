...
import { authorizeJcb } from "@/lib/integrations/jcb/server";
...
describe("LiveLink access boundary", () => {
  beforeEach(() => { process.env.JCB_LIVELINK_ENABLED = "true"; });
  afterEach(() => { delete process.env.JCB_LIVELINK_ENABLED; vi.clearAllMocks(); });
  ...
  it("fails closed until explicitly enabled", async () => { client("admin", false); delete process.env.JCB_LIVELINK_ENABLED; await expect(authorizeJcb(request)).rejects.toMatchObject({ status: 503 }); });
