...
function getRealtimePayloadField(
  record: unknown,
  field: string,
) {
...
export default function AdminPage() {
  ...
  const realtimeTicketIdsRef = useRef<Set<string>>(new Set());
  const realtimeMessagesByTicketIdsRef = useRef<Set<string>>(new Set());
  const realtimeBatchTimeoutRef = useRef<number | null>(null);
  ...
  const refreshRequesterMessagesForTicketIds = useCallback(async (
    ticketIds: string[],
  ) => {
    ...
    setRequesterMessagesByTicket((current) => {
      const next = { ...current };

      uniqueTicketIds.forEach((ticketId) => {
        const nextMessages = grouped[ticketId] ?? [];
  ...
  const hydrateOperationalTicketIds = useCallback(async (
    ticketIds: string[],
  ) => {
    const uniqueTicketIds = Array.from(new Set(ticketIds.filter(Boolean)));
    ...
    const [{ data: ticketRows, error: ticketError }, updatesResult] = await Promise.all([
      supabase
        .from("tickets")
        .select("*")
    ...
    const nextTicketMap = new Map(
      ((ticketRows ?? []) as Ticket[])
        .filter((ticket) => Boolean(ticket.id))
        .map((ticket) => [ticket.id, ticket]),
    );
    const nextActiveTickets = Array.from(nextTicketMap.values()).filter((ticket) =>
      isActiveTicketStatusValue(ticket.status),
    );
    const avatarUserIds = nextActiveTickets
      .map((ticket) => ticket.user_id)
      .filter((userId): userId is string => Boolean(userId));
    const avatarMap =
      avatarUserIds.length > 0
        ? await fetchProfileAvatarUrls(supabase, avatarUserIds).catch(() => ({} as Record<string, string | null>))
        : {};
    const updateRows = updatesResult.error
      ? []
      : ((updatesResult.data ?? []) as Array<{ ticket_id: string; comment: string | null }>);
    ...
    const returnedReasons = updateRows.reduce<Record<string, string>>((accumulator, update) => {
    ...
    setTickets((current) => {
      ...
      uniqueTicketIds.forEach((ticketId) => {
        const nextTicket = nextTicketMap.get(ticketId);

        if (!nextTicket || !isActiveTicketStatusValue(nextTicket.status)) {
          currentMap.delete(ticketId);
          return;
        }

        currentMap.set(ticketId, {
          ...(currentMap.get(ticketId) ?? {}),
          ...nextTicket,
        });
      });
    ...
    setCollectedTicketIds((current) => {
      const next = new Set(current);

      uniqueTicketIds.forEach((ticketId) => {
        next.delete(ticketId);

        if (collectedIds.has(ticketId)) {
          next.add(ticketId);
        }
      });
    ...
    setReturnedTicketReasonById((current) => {
      const next = { ...current };

      uniqueTicketIds.forEach((ticketId) => {
        const nextReason = returnedReasons[ticketId];

        if (nextReason) {
          next[ticketId] = nextReason;
        } else {
          delete next[ticketId];
        }
      });
    ...
    if (Object.keys(avatarMap).length > 0) {
      setProfileAvatarByUserId((current) => ({
        ...current,
        ...avatarMap,
      }));
    }
    ...
    setOrders((current) => {
      ...
      uniqueTicketIds.forEach((ticketId) => {
        const nextTicket = nextTicketMap.get(ticketId);
        const nextIsTrackedOrder = nextTicket ? isTrackedOrderRecord(nextTicket) : false;
        const allowedStatuses = new Set(getOrdersFilterStatuses(ordersFilter));
        const matchesOrdersFilter =
        ...
        if (!nextTicket || !nextIsTrackedOrder || !matchesOrdersFilter) {
          currentMap.delete(ticketId);
          return;
        }
    ...
    if (selectedChatTicketId && uniqueTicketIds.includes(selectedChatTicketId)) {
      void reloadSelectedChatMessages(supabase, selectedChatTicketId).catch(() => {});
    }
  }, [ordersFilter, selectedChatTicketId]);

  const flushRealtimeBatch = useCallback(async () => {
    realtimeBatchTimeoutRef.current = null;

    const ticketIds = Array.from(realtimeTicketIdsRef.current);
    const messageTicketIds = Array.from(realtimeMessagesByTicketIdsRef.current);

    realtimeTicketIdsRef.current.clear();
    realtimeMessagesByTicketIdsRef.current.clear();

    if (ticketIds.length > 0) {
      await hydrateOperationalTicketIds(ticketIds);
    }

    if (messageTicketIds.length > 0) {
      await refreshRequesterMessagesForTicketIds(messageTicketIds);
    }
  }, [hydrateOperationalTicketIds, refreshRequesterMessagesForTicketIds]);

  const scheduleRealtimeBatch = useCallback(() => {
    if (realtimeBatchTimeoutRef.current != null) {
      return;
    }

    realtimeBatchTimeoutRef.current = window.setTimeout(() => {
      void flushRealtimeBatch();
    }, ADMIN_REALTIME_BATCH_DELAY_MS);
  }, [flushRealtimeBatch]);
  ...
  useEffect(() => {
    ...
    const supabase = getSupabaseClient();
    const realtimeTicketIds = realtimeTicketIdsRef;
    const realtimeMessageTicketIds = realtimeMessagesByTicketIdsRef;

    if (!supabase) {
      return;
    }

    const queueChannel = supabase.channel("relay-admin-live-operations");
    ...
    queueChannel.on(
      ...
      (payload) => {
        const ticketId =
          getRealtimePayloadField(payload.new, "id")
            ? getRealtimePayloadField(payload.new, "id")
            : getRealtimePayloadField(payload.old, "id")
              ? getRealtimePayloadField(payload.old, "id")
              : null;
        ...
        realtimeTicketIds.current.add(ticketId);
        scheduleRealtimeBatch();
      },
    );
    ...
    queueChannel.on(
      ...
      (payload) => {
        const ticketId =
          getRealtimePayloadField(payload.new, "ticket_id")
            ? getRealtimePayloadField(payload.new, "ticket_id")
            : getRealtimePayloadField(payload.old, "ticket_id")
              ? getRealtimePayloadField(payload.old, "ticket_id")
              : null;
    ...
    queueChannel.on(
      "postgres_changes",
      {
        ...
        filter: "sender_role=eq.requester",
      },
      (payload) => {
        const ticketId =
          getRealtimePayloadField(payload.new, "ticket_id")
        ...
        realtimeMessageTicketIds.current.add(ticketId);
        scheduleRealtimeBatch();
      },
    );
    ...
    return () => {
      const pendingBatchTimeout = realtimeBatchTimeoutRef.current;
      const pendingTicketIds = realtimeTicketIds.current;
      const pendingMessageTicketIds = realtimeMessageTicketIds.current;

      document.removeEventListener("visibilitychange", handleVisibilityRefresh);

      if (pendingBatchTimeout != null) {
        window.clearTimeout(pendingBatchTimeout);
        realtimeBatchTimeoutRef.current = null;
      }

      pendingTicketIds.clear();
      pendingMessageTicketIds.clear();
