...
export async function GET(request: NextRequest) {
  try {
    const auth = await authorizeTrackunit(request, true);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (change.action === "access") {
      ...
      const fleet = await getTrackunitFleet();
      if (!fleet.machines.some(m => m.pin === change.pin)) throw new JcbError("Machine not found in Manitou Track.", 404);
      if (change.machineId === null) {
        const result = await auth.supabase.from("trackunit_mappings").delete().eq("pin", change.pin);
        ...
        const result = await auth.supabase.from("trackunit_mappings").upsert({ pin: change.pin, machine_id: change.machineId, updated_by: auth.user.id, updated_at: new Date().toISOString() });
        if (result.error) throw new JcbError("Unable to link this machine. It may already be linked to another Manitou PIN.", 409);
      }
    }
