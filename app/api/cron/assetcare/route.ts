...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const saved=await db.from('assetcare_stream_state').update({owner:null,lease_until:null,last_error:null,...(result.drained?{last_ack_at:new Date().toISOString()}:{}),last_cycle:result,next_allowed_at:new Date(Date.now()+(result.drained?60000:5000)).toISOString()}).eq('id',true).eq('owner',owner).select('id');
