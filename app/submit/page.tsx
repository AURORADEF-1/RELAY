...
export default function SubmitPage() {
  ...
  useEffect(() => {
    if (!hasHydratedOfflineDraftRef.current) return;
  ...
  useEffect(() => {
    let isMounted = true;
    void countRequesterOfflineSubmissions()
      .then((count) => {
        if (isMounted) setOfflineQueueCount(count);
      })
      .catch(() => {
        if (isMounted) setOfflineQueueCount(0);
      });
  ...
  const syncOfflineQueue = useCallback(async () => {
    if (isSyncingOfflineQueueRef.current) return;
    ...
    try {
      ...
      for (const submission of mine) {
        ...
        try {
          let registryRecord: MachineRegistryRecord | null = null;
          if (submission.values.machineReference.trim()) {
            try {
              registryRecord = await lookupMachineRegistryRecord(
                supabase,
          ...
          const ticketPayload = buildTicketInsertPayload({
            ...
            machineRegistryRecord: registryRecord,
          });
          ...
          if (submission.phase === "create-ticket" || !ticketId) {
            ...
            const { error: updateError } = await supabase
              .from("ticket_updates")
              .insert({ ticket_id: ticketId, status: "PENDING", comment: "Ticket created from offline queue." });
            if (updateError) {
              console.error("Offline ticket status log failed", updateError);
            }
          ...
          if (!notified && ticketId) {
            await notifyAdminsOfNewTicket(supabase, {
              ...
              console.error("Failed to notify admins of saved offline request", notificationError);
            });
            notified = true;
            await updateRequesterOfflineSubmission(submission.id, {
              notified,
          ...
          const message = error instanceof Error ? error.message : "Unable to sync the saved request.";
          ...
          setOfflineStatusMessage({
            type: "error",
            message: `A saved request needs attention before it can sync: ${message}`,
          });
        }
      }
  ...
  return (
    <section className="aurora-panel rounded-[1.6rem] border-[color:var(--border)] bg-[color:var(--background-panel-strong)] p-4">
      ...
      >
        ...
        <span className="text-xs font-semibold uppercase tracking-[0.22em] text-[color:var(--foreground-muted)]">
          {isOpen ? "Hide" : "View"}
        </span>
      </button>
